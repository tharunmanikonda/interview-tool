...
Interview Tool is a prototype live interview assistant built from three pieces:
...
- A local realtime Docs viewer server that creates read-only share links.
...
│   ├── docs-server     # Local realtime read-only Docs viewer server
...
- Read-only realtime Docs share links.
...
Then open ChatGPT with the extension loaded and click `Share session` in the top bar. The extension creates a local read-only viewer link and copies it to your clipboard.

The viewer runs at:

```text
http://127.0.0.1:8787/s/<view-token>
```

Anyone who can reach that local server and has the link can watch the current transcript, question cards, answer stream, attachments, and latency status. V1 viewers cannot edit, send prompts, or control ChatGPT.
...
- `apps/docs-server/src/server.ts` - realtime read-only Docs session server.
- `apps/docs-server/public` - hosted viewer UI.
