...
export class TabAudioCaptureProvider {
  ...
  async start(callbacks: TabAudioCallbacks) {
    ...
    const response = await safeRuntimeMessage<{ ok?: boolean; streamId?: string; error?: string }>({ type: "gptd:get-tab-audio-stream-id" });
...
async function safeRuntimeMessage<T>(message: unknown): Promise<T | undefined> {
  try {
    if (!chrome?.runtime?.id) return undefined;
    return await chrome.runtime.sendMessage(message) as T;
  } catch (error) {
    const text = error instanceof Error ? error.message : String(error || "");
    if (text.includes("Extension context invalidated") || text.includes("context invalidated")) return undefined;
    throw error;
  }
}
